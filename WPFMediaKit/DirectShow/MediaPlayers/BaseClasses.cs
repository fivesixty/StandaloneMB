...
namespace WPFMediaKit.DirectShow.MediaPlayers
{
    ...
    public class HiddenWindow : NativeWindow
    {
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            lock (((System.Collections.ICollection)m_handlerlist).SyncRoot)
            {
                foreach (WndProcHookDelegate m_method in m_handlerlist)
                {
                    m_method.Invoke(m.HWnd, m.Msg, m.WParam, m.LParam, ref IsHandled);
                    if (IsHandled)
                        break;
                }
            }
    ...
    /// </summary>
    public abstract class MediaPlayerBase : DispatcherObject
    {
        /// <summary>
        ...
        /// </summary>
        private static int GRAPH_INSTANCES;
        ...
        /// </summary>
        private const int DSHOW_TIMER_POLL_MS = 100;

        /// <summary>
        /// UserId value for the VMR9 Allocator - Not entirely useful
        /// for this application of the VMR
        /// </summary>
        private readonly IntPtr userId = new IntPtr(unchecked((int)0xDEADBEEF));
        ...
        /// <summary>
        /// This is the polling timer we use to do periodic
        /// tasks with the graph, such as getting the
        /// current graph's media position
        /// </summary>
        private DispatcherTimer m_dShowTimer;
        ...
        /// </summary>
        private static HiddenWindow m_window;
        ...
        protected int GraphInstanceId
        {
            get
            {
                ...
                m_graphInstanceId = Interlocked.Increment(ref GRAPH_INSTANCES);
        ...
        /// <summary>
        /// The private handler for the DispatcherTimer tick
        /// </summary>
        private void OnGraphTimerPrivate(object sender, EventArgs e)
        {
            OnGraphTimerTick();
        }
        ...
        {
            if (m_dShowTimer == null)
            {
                m_dShowTimer = new DispatcherTimer
                {
                    Interval = TimeSpan.FromMilliseconds(DSHOW_TIMER_POLL_MS)
                };

                m_dShowTimer.Tick += OnGraphTimerPrivate;
            }

            /* Make sure we get windows messages */
            AddWndProcHook();

            if (!m_dShowTimer.IsEnabled)
                m_dShowTimer.Start();
        }
        ...
        {
            /* Make sure this is a graph message and make sure it's intended for our graph */
            if (msg == WM_GRAPH_NOTIFY && lParam == (IntPtr)GraphInstanceId)
            {
                IntPtr param1;
                IntPtr param2;
                EventCode code;
                ...
                while (m_mediaEvent.GetEvent(out code, out param1, out param2, 0) == 0)
                {
                    ...
                    /* Copy the params over for our BeginInvoke */
                    IntPtr lparam1 = param1;
                    IntPtr lParam2 = param2;

                    /* Handle anything for this event code */
                    Dispatcher.BeginInvoke((Action)(() => OnMediaEvent(eventCode, lparam1, lParam2)));
                }
            }
        ...
        private void UnsetMediaEventExNotifyWindow()
        {
            ...
            m_mediaEvent.SetNotifyWindow(IntPtr.Zero, WM_GRAPH_NOTIFY, (IntPtr)GraphInstanceId);
        }
        ...
        /// </summary>
        private void CustomAllocator_NewAllocatorSurface(object sender, IntPtr pSurface)
        {
        ...
        /// </summary>
        private void CustomAllocator_NewAllocatorFrame()
        {
        ...
        {
            /* Probably a good place to stop the 
             * polling timer */
            StopGraphPollTimer();
        ...
        {
            var Delegate = NewAllocatorSurface;
            if (Delegate != null)
                Delegate(this, pSurface);
        }
        ...
        protected static IBaseFilter AddFilterByName(IGraphBuilder graphBuilder, Guid deviceCategory, string friendlyName)
        {
            ...
            for (int i = 0; i < devices.Length; i++)
            {
                ...
                int hr = filterGraph.AddSourceFilterForMoniker(devices[i].Mon, 
                                                               null, 
                                                               friendlyName, 
                                                               out filter);
                DsError.ThrowExceptionForHR(hr);
